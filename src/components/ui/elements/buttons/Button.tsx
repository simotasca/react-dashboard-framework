...
const styles = {
  base: [
    ...
    // Icon
    // "[&>[data-slot=icon]]:-mx-0.5 [&>[data-slot=icon]]:my-0.5 [&>[data-slot=icon]]:sm:my-1",
    // "[&>[data-slot=icon]]:size-5 [&>[data-slot=icon]]:sm:size-4 [&>[data-slot=icon]]:shrink-0",
    // "[&>[data-slot=icon]]:text-[--btn-icon] forced-colors:[--btn-icon:ButtonText] forced-colors:data-[hover]:[--btn-icon:ButtonText]",
  ],
  icon: [
    "[&>[data-slot=icon]]:-mx-0.5 [&>[data-slot=icon]]:my-0.5 [&>[data-slot=icon]]:sm:my-1",
    "[&>[data-slot=icon]]:size-5 [&>[data-slot=icon]]:sm:size-4 [&>[data-slot=icon]]:shrink-0",
...
type ButtonProps = React.PWC<
  & Variants 
  & Omit<h.ButtonProps, "className">
  & { className?: ClassNameValue; icon?: boolean }
>;

export const Button = forwardRef((
  { icon = true, color = "zinc", outline, plain, className, children, ...props }: ButtonProps,
  ...
  const classes = cn(
    ...
    outline
      ...
      : plain 
      ? styles.plain
      : twJoin(styles.solid, styles.colors[color]),
    className,
  );
  return (
    <h.Button {...props} className={classes} ref={ref}>
