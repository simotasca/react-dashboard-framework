...
type Variants = { color?: keyof typeof colors };

type BadgeProps = Variants & React.ComponentPropsWithoutRef<"span">;
...
export type BadgeButtonProps = Variants & { className?: string; children: React.ReactNode } & Omit<h.ButtonProps, "className">;
export const BadgeButton = forwardRef(
  (
    { color, className, children, ...props }: BadgeButtonProps,
    ...
    return (
      <h.Button {...props} className={classes} ref={ref}>
        <TouchTarget>
          <Badge color={color ?? "zinc"}>{children}</Badge>
        </TouchTarget>
      </h.Button>
    );
  }
